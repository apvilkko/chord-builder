...
import ChordBuilder from '../src/builder';

const verify = (expected, notes) =>
  expect(map('vexNote')(notes)).to.eql(expected);
...
describe('Builder', () => {
  it('should set notes for Vex', () => {
    ...
    expect(vexNotes).to.eql(expected);
  });
  ...
  it('should handle guitar chords', () => {
    ...
    parser.parse('E');
    builder.buildChord(parser.model);
    expect(parser.model.guitar).to.be.equal('0-2-2-1-0-0');

    parser.parse('dsus4');
    builder.buildChord(parser.model);
    expect(parser.model.guitar).to.be.equal('x-x-0-2-3-3');

    parser.parse('A');
    builder.buildChord(parser.model);
    expect(parser.model.guitar).to.be.equal('x-0-2-2-2-0');

    parser.parse('Asus2');
    builder.buildChord(parser.model);
    expect(parser.model.guitar).to.be.equal('x-0-2-2-0-0');

    parser.parse('Asus4');
    builder.buildChord(parser.model);
    expect(parser.model.guitar).to.be.equal('x-0-2-2-3-0');

    parser.parse('g#maj7');
    builder.buildChord(parser.model);
    expect(parser.model.guitar).to.be.equal('4-x-1-1-1-3');
  });
  ...
  it('should handle a bunch of chords', () => {
    ...
    parser.parse('Bb13');
    builder.buildChord(parser.model);
    verify(['Bb/3', 'D/4', 'F/4', 'Ab/4', 'C/5', 'Eb/5', 'G/5'],
     parser.model.notes);

    parser.parse('Eadd4/F#');
    builder.buildChord(parser.model);
    expect(parser.model.bassNote.vexNote).to.be.equal('F#/3');
